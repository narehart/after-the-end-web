...
# from https://raw.githubusercontent.com/MattUV/godot-MaxSizeContainer/master/addons/MaxSizeContainer/max_size_container.gd
...
func _ready() -> void:
  ...
  var _id = connect("resized", self, "_on_self_resized")
...
func _initialize(p_child: Node) -> void:
  ...
  if not child.is_connected("tree_exiting", self, "_on_child_tree_exiting"):
    var _id = child.connect("tree_exiting", self, "_on_child_tree_exiting")
  if not child.is_connected("minimum_size_changed", self, "_on_child_minimum_size_changed"):
    var _id = child.connect("minimum_size_changed", self, "_on_child_minimum_size_changed")
  if not child.is_connected("tree_entered", self, "_on_child_tree_entered"):
    var _id = child.connect("tree_entered", self, "_on_child_tree_entered")
